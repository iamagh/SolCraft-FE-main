    ...
    spawn.stderr.on("data", function (data: string) {
      ...
      if (data.includes("64") && data.includes("1.8")) {
        ...
        if (javaVersion != false) {
          console.log("Java " + javaVersion + " is already installed");
          jre = "default";
          return resolve();
        } else {
  ...
  try {
    ...
    data.on("end", async function () {
      ...
      if (
        ...
        console.log("Java installation successfully downloaded!");
        const zip = new AdmZip(targetPath);
        console.log("Extracting java!");
        setUpdateText("ExtractingJava");
        zip.extractAllTo(jrePath, true);
        console.log("Java was successfully extracted!");
        fs.unlink(targetPath, (err) => {
          if (err) {
            console.error(err.message);
            return;
          }
          console.log("Java installation file was successfully removed!");
        });
        jre = jrePath;
        updateAndLaunch();
      } else {
        console.error("Error while downloading java!");
      ...
      if (
        ...
        data.on("end", async function () {
          if (fs.statSync(forgeInstallerFile).size == totalLength) {
            console.log("Forge was successfully downloaded!");
            resolve();
          } else {
            reject();
          }
        ...
        console.log("Forge installer is already installed");
        resolve();
      }
    } catch (err: any) {
      console.error(err.message);
