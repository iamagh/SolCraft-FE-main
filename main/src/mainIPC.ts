...
function initMainIPC() {
  //Async utils
  ipc.on("open-link", (_event, args) => shell.openExternal(args));
  ipc.on("ping-server", () => {
  ...
  ipc.on("set-memory", (_event, args) => {
    const memory = args + "G";
    configManager.setMinRAM(memory);
    configManager.setMaxRAM(memory);
    configManager.saveConfig();
  });
  ipc.on("set-auto-auth", (_event, args) => {
    configManager.setAutoAuthEnabled(args);
    configManager.saveConfig();
  });
  ipc.on("set-keep-launcher-open", (_event, args) => {
